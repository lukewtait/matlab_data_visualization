function pl = plot_data(x,varargin) ; 
...
% To do: 
% - Add font size
% - Add ability to edit axes properties - set default axis line width to
%   boxplot line width
% - Add a preset default called 'matlab_boxplot' to print it as a Matlab
%   format boxplot (red blue lines etc)
...
% Plot distributions --------------------
if any(pl.plotDist)
    % initialize plot of distributions
    yq = nan(size(pl.Distribution_Mesh)) ; 
    
    % loop over all groups
    for i = 1:Ngroups

        if ~pl.plotDist(i) ; continue ; end
        % Get data from group i 
        idx = find(pl.Groups == grouplbls(i)) ; 
        xm = pl.Data(idx) ; 

        % Distribution plot
        if pl.plotDist(i)
            if strcmp(pl.Distribution_Name{i},'kernel') & ~strcmp(pl.Distribution_BandWidth{i},'auto')
                dst = fitdist(xm,pl.Distribution_Name{i},'width',pl.Distribution_BandWidth{i}) ;
            else
                dst = fitdist(xm,pl.Distribution_Name{i}) ; 
                pl.Distribution_BandWidth{i} = dst.BandWidth ; 
            end 
            yq(i,:) = pdf(dst,pl.Distribution_Mesh(i,:)) ; 
        end
        
    end

    % rescale from 0-1
    mYq = min(yq(:)); MYq = max(yq(:)) ; 
    switch pl.Distribution_Normalization{1}
        case 'all'
            yq = yq-min(yq(:)) ; yq = yq/max(yq(:)) ; 
        case 'group'
            yq = yq-min(yq,[],2) ; yq = yq./max(yq,[],2) ; 
        otherwise
            error('Distribution_Normalization must be either all or group')
    end
        
    
    % align to left or right hand side
    switch pl.Distribution_Side{1}
        case 'violin'
            yq = [yq' ; flipud(yq')]' ; 
            pl.Distribution_Mesh = [pl.Distribution_Mesh' ; flipud(pl.Distribution_Mesh')]' ; 
            for i = 1:Ngroups
                if ~pl.plotDist(i) ; continue ; end
                yq(i,1:size(yq,2)/2)              = pl.GroupXAxis(i)+pl.Distribution_GridBox(i,1)+diff(pl.Distribution_GridBox(i,:))/2+pl.ShiftGrid(i) - 0.5*diff(pl.Distribution_GridBox(i,:))*yq(i,1:size(yq,2)/2)  ; 
                yq(i,(size(yq,2)/2+1):size(yq,2)) = pl.GroupXAxis(i)+pl.Distribution_GridBox(i,1)+diff(pl.Distribution_GridBox(i,:))/2+pl.ShiftGrid(i) + 0.5*diff(pl.Distribution_GridBox(i,:))*yq(i,(size(yq,2)/2+1):size(yq,2)) ; 
            end
        case 'left'
            % align to left
            for i = 1:Ngroups
                if ~pl.plotDist(i) ; continue ; end
                yq(i,:) = pl.GroupXAxis(i)+pl.Distribution_GridBox(i,2)+pl.ShiftGrid(i) - diff(pl.Distribution_GridBox(i,:))*yq(i,:) ; 
            end
            % fit ones at end so it goes back to zero
            pl.Distribution_Mesh = [pl.Distribution_Mesh(:,1) , pl.Distribution_Mesh , pl.Distribution_Mesh(:,end)] ; 
            yq = [pl.GroupXAxis' + pl.Distribution_GridBox(:,2)+pl.ShiftGrid(i) , yq , pl.GroupXAxis' + pl.Distribution_GridBox(:,2)+pl.ShiftGrid(i)] ; 
        case 'right'
            % align to right
            for i = 1:Ngroups
                if ~pl.plotDist(i) ; continue ; end
                yq(i,:) = pl.GroupXAxis(i)+pl.Distribution_GridBox(i,1)+pl.ShiftGrid(i) + diff(pl.Distribution_GridBox(i,:))*yq(i,:) ; 
            end
            % fit ones at end so it goes back to zero
            pl.Distribution_Mesh = [pl.Distribution_Mesh(:,1) , pl.Distribution_Mesh , pl.Distribution_Mesh(:,end)] ; 
            yq = [pl.GroupXAxis' + pl.Distribution_GridBox(:,1)+pl.ShiftGrid(i) , yq , pl.GroupXAxis' + pl.Distribution_GridBox(:,1)+pl.ShiftGrid(i)] ; 
        otherwise
            error('Distribution_Side may be left or right')
    end
    
    % plot faces of distributions
    for i = 1:Ngroups
        if ~pl.plotDist(i) ; continue ; end
        
        if pl.Distribution_PlotFace(i)
            pl.handle.distface(i) = fill(yq(i,:),pl.Distribution_Mesh(i,:),pl.Distribution_FaceColor(i,:),'LineStyle','none') ; 
        end
    end
    
    % remove additional ones at ends
    pl.Distribution_Mesh = pl.Distribution_Mesh(:,2:end-1) ; 
    yq = yq(:,2:end-1) ; 
    
    % plot edges of distributions
    for i = 1:Ngroups
        if ~pl.plotDist(i) ; continue ; end
        
        if pl.Distribution_PlotEdge(i)
            pl.handle.distedge(i) = plot(yq(i,:),pl.Distribution_Mesh(i,:),'Color',pl.Distribution_LineColor(i,:),'LineWidth',pl.Distribution_LineWidth(i,:)) ; 
        end
    end
    
    % plot calibration
    if ~isempty(pl.Distribution_CalibrationGroup) && strcmp(pl.Distribution_Normalization{1},'all')
        i = pl.Distribution_CalibrationGroup ;
        plot(i+pl.ShiftGrid(i)+pl.Distribution_GridBox(i,:),repmat(pl.Distribution_CalibrationHeight,1,2),'k','LineWidth',pl.AxesLineWidth) ; 
        plot(repmat(i+pl.ShiftGrid(i)+pl.Distribution_GridBox(i,1),1,2),pl.Distribution_CalibrationHeight+[0,-0.02*range(pl.Distribution_Mesh(:))],'k','LineWidth',pl.AxesLineWidth)
        plot(repmat(i+pl.ShiftGrid(i)+pl.Distribution_GridBox(i,2),1,2),pl.Distribution_CalibrationHeight+[0,-0.02*range(pl.Distribution_Mesh(:))],'k','LineWidth',pl.AxesLineWidth)
        switch pl.Distribution_Side{1}
            case 'left'
                text(i+pl.ShiftGrid(i)+pl.Distribution_GridBox(i,2),pl.Distribution_CalibrationHeight-0.02*range(pl.Distribution_Mesh(:)),sprintf('\n%.2f',mYq),'FontSize',pl.FontSize,'HorizontalAlignment','center')
                text(i+pl.ShiftGrid(i)+pl.Distribution_GridBox(i,1),pl.Distribution_CalibrationHeight-0.02*range(pl.Distribution_Mesh(:)),sprintf('\n%.2f',MYq),'FontSize',pl.FontSize,'HorizontalAlignment','center')
                text(i+pl.ShiftGrid(i)+mean(pl.Distribution_GridBox(i,:)),pl.Distribution_CalibrationHeight-0.02*range(pl.Distribution_Mesh(:)),sprintf('\n\nPDF'),'FontSize',pl.FontSize,'HorizontalAlignment','center')
        end
    end
end
% Plot distributions done ----------------------
...
if any(pl.plotBox)
    ...
    for i = 1:Ngroups
        ...
        pl.handle.box(i) = rectangle('Position',[pl.GroupXAxis(i)+pl.Box_GridBox(i,1)+pl.ShiftGrid(i) , y(1) , diff(pl.Box_GridBox(i,:)) , diff(y)],'curvature',pl.Box_Curvature(i,:),'EdgeColor',pl.Box_BoxLineColor(i,:),'FaceColor',pl.Box_FaceColor(i,:),'LineWidth',pl.Box_BoxLineWidth(i)) ; 
        % Add median
        pl.handle.median(i) = plot(pl.GroupXAxis(i)+pl.Box_GridBox(i,:)+pl.ShiftGrid(i),[m,m],'Color',pl.Box_MedianLineColor(i,:),'LineWidth',pl.Box_MedianLineWidth(i)) ; 
        % Add whiskers
        pl.handle.whiskersmin(i) = plot([pl.GroupXAxis(i),pl.GroupXAxis(i)]+mean(pl.Box_GridBox(i,:))+pl.ShiftGrid(i),[mx,y(1)],'Color',pl.Box_WhiskerLineColor(i,:),'LineWidth',pl.Box_WhiskerLineWidth(i)) ; % lower
        pl.handle.whiskersmax(i) = plot([pl.GroupXAxis(i),pl.GroupXAxis(i)]+mean(pl.Box_GridBox(i,:))+pl.ShiftGrid(i),[y(2),Mx],'Color',pl.Box_WhiskerLineColor(i,:),'LineWidth',pl.Box_WhiskerLineWidth(i)) ;  % upper
        % Add whisker caps
        pl.handle.whiskermincaps(i) = plot([pl.GroupXAxis(i),pl.GroupXAxis(i)]+mean(pl.Box_GridBox(i,:))+pl.ShiftGrid(i)+pl.Box_WhiskerCapWidth(i)/2*[-1,1],[mx,mx],'Color',pl.Box_WhiskerLineColor(i,:),'LineWidth',pl.Box_WhiskerLineWidth(i)) ; 
        pl.handle.whiskermaxcaps(i) = plot([pl.GroupXAxis(i),pl.GroupXAxis(i)]+mean(pl.Box_GridBox(i,:))+pl.ShiftGrid(i)+pl.Box_WhiskerCapWidth(i)/2*[-1,1],[Mx,Mx],'Color',pl.Box_WhiskerLineColor(i,:),'LineWidth',pl.Box_WhiskerLineWidth(i)) ; 
    end
%     % Add points
%     if pl.PlotPoints
%         pl.handle.points(:,i) = plot(repmat(pl.PointsLocations(i),length(tempx{i}),1),tempx{i},'.','Color',pl.PointsColors(i,:),'MarkerSize',pl.PointsMarkerSize(i)) ; 
%     end
end
% Plot box and whisker done ---------------------------



% Plot points ----------------------------------------
if any(pl.plotPoints)
    % Loop over groups
    for i = 1:Ngroups
        
        if ~pl.plotPoints(i) ; continue ; end
        
        idx = find(pl.Groups == grouplbls(i)) ; 
        xm = x(idx) ;
        
        % draw plot point locations from within the grid
        y = pl.GroupXAxis(i)+pl.ShiftGrid(i)+pl.Points_GridBox(i,1)+diff(pl.Points_GridBox(i,:))*rand(length(xm),1) ; 
        ...
        pl.handle.points(:,i) = plot(y,xm,pl.Points_Marker{i},'Color',pl.Points_MarkerLineColor(i,:),'MarkerFaceColor',facecol,'MarkerSize',pl.Points_MarkerSize(i)) ; 
    end
end
...
xlim([min(pl.GroupXAxis'+pl.ShiftGrid) , max(pl.GroupXAxis'+1+pl.ShiftGrid)])
...
function [x,pl,Ngroups] = check_inputs(x,inpl)
    ...
    if any(pl.plotDist)
        pli = struct ; 
        pli.Distribution_Name = 'kernel' ; 
        for m = 1:Ngroups
            xm = pl.Data(pl.Groups == m) ; 
            pli.Distribution_Mesh(m,:) = linspace(min(xm)-(1/3)*(max(xm)-min(xm)),max(xm)+(1/3)*(max(xm)-min(xm)),100) ; 
        end
        if ~isviolin
            pli.Distribution_Side = 'left' ; 
            pli.Distribution_GridBox = [0.1,0.45] ;
        else
            pli.Distribution_Side = 'violin' ; 
            pli.Distribution_GridBox = [0.1,0.9] ; 
        end
        pli.Distribution_PlotFace = true ; 
        pli.Distribution_FaceColor = pl.GroupColors ;  
        pli.Distribution_PlotEdge = true ; 
        pli.Distribution_LineColor = pl.plotLineColor ; 
        pli.Distribution_LineWidth = pl.plotLineWidth ; 
        pli.Distribution_BandWidth = 'auto' ; 
        [pli,inpl] = update_options(pli,inpl) ; % update
        pl = insert_plotpl(pl,pli,Ngroups) ; 
        
        pl.Distribution_Normalization = 'all' ; 
        pl.Distribution_CalibrationGroup = [] ; 
        pl.Distribution_CalibrationHeight = min(pl.Distribution_Mesh(:)) + 0.15*range(pl.Distribution_Mesh(:)) ;
        [pl,inpl] = update_options(pl,inpl) ; % update
    else
        pli = struct ; 
        pli.Distribution_Name = 'none' ; 
        for m = 1:Ngroups
            xm = pl.Data(pl.Groups == m) ; 
            pli.Distribution_Mesh(m,:) = nan ; 
        end
        if ~isviolin
            pli.Distribution_Side = 'none' ; 
            pli.Distribution_GridBox = [0,0] ;
        else
            error('Cannot select violin plot with plotDist=false')
        end
        pli.Distribution_PlotFace = false ; 
        pli.Distribution_FaceColor = 'none' ;  
        pli.Distribution_PlotEdge = false ; 
        pli.Distribution_LineColor = 'none' ; 
        pli.Distribution_LineWidth = 'none' ; 
        pli.Distribution_BandWidth = 'none' ; 
        [pli,inpl] = update_options(pli,inpl) ; % update
        pl = insert_plotpl(pl,pli,Ngroups) ; 
        
        pl.Distribution_Normalization = 'none' ; 
        pl.Distribution_CalibrationGroup = 'none' ; 
        pl.Distribution_CalibrationHeight = 'none' ;
        [pl,inpl] = update_options(pl,inpl) ; % update
    end
    % distribution plot options done ---
    
    % Points options --- 
    if any(pl.plotPoints)
        pli = struct ; 
        pli.Points_MarkerSize = 4 ; 
        pli.Points_MarkerFaceColor = 'none' ; 
        pli.Points_MarkerLineColor = pl.plotLineColor ; 
        pli.Points_Marker = '.' ; 
        pli.Points_GridBox = [0.85,0.9] ;  
        [pli,inpl] = update_options(pli,inpl) ; % update here as variables depend on this later
        pl = insert_plotpl(pl,pli,Ngroups) ;
    else
        pli = struct ; 
        pli.Points_MarkerSize = 'none' ; 
        pli.Points_MarkerFaceColor = 'none' ; 
        pli.Points_MarkerLineColor = 'none' ; 
        pli.Points_Marker = 'none' ; 
        pli.Points_GridBox = [0,0] ;  
        [pli,inpl] = update_options(pli,inpl) ; % update here as variables depend on this later
        pl = insert_plotpl(pl,pli,Ngroups) ;
    end
    % Points options done --- 
    ...
    if any(pl.plotBox)
        ...
        pli.Box_MedianFun = @(x) median(x) ;
        if ~isviolin
            pli.Box_WhiskerMinFun = @(x) min(x) ; 
            pli.Box_WhiskerMaxFun = @(x) max(x) ;
        else
            pli.Box_WhiskerMinFun = @(x) median(x)-1.5*iqr(x) ; 
            pli.Box_WhiskerMaxFun = @(x) median(x)+1.5*iqr(x) ; 
        end

        % locations
        if ~isviolin
            pli.Box_GridBox = [0.52,0.78]; % median(pl.Points_GridBox,2) + (1/3)*diff(pl.Points_GridBox,[],2)*[-1,1] ;
            pli.Box_FaceColor = pl.GroupColors ; 
        else
            pli.Box_GridBox = [0.45,0.55] ; 
        ...
        pli.Box_MedianLineColor = pl.plotLineColor ; 
        pli.Box_MedianLineWidth = pl.plotLineWidth ; 
        ...
        pli = struct ; 
        pli.Box_BoxFun = 'none' ;
        pli.Box_MedianFun = 'none' ;
        if ~isviolin
            pli.Box_WhiskerMinFun = 'none' ; 
            pli.Box_WhiskerMaxFun = 'none' ;
        else
            pli.Box_WhiskerMinFun = 'none' ; 
            pli.Box_WhiskerMaxFun = 'none' ; 
        end
        ...
        % locations
        if ~isviolin
            pli.Box_GridBox = [0,0]; % median(pl.Points_GridBox,2) + (1/3)*diff(pl.Points_GridBox,[],2)*[-1,1] ;
            pli.Box_FaceColor = 'none' ; 
        else
            pli.Box_GridBox = [0,0] ; 
            pli.Box_FaceColor = 'none' ; 
        end
        pli.Box_Curvature = 'none' ; 
        pli.Box_BoxLineColor = 'none' ;
        pli.Box_BoxLineWidth = 'none' ;
        pli.Box_MedianLineColor = 'none' ; 
        pli.Box_MedianLineWidth = 'none' ; 
        pli.Box_WhiskerLineColor = 'none' ; 
        pli.Box_WhiskerLineWidth = 'none' ; 
        [pli,inpl] = update_options(pli,inpl) ; % update here as variables depend on this later
        pli.Box_WhiskerCapWidth = 'none' ; % 0.3*diff(pli.Box_GridBox,[],2) ; 
        [pli,inpl] = update_options(pli,inpl) ; % update here as variables depend on this later
    ...
    % Shift grid 
    if ~isviolin
        pl.ShiftGrid = -(pl.Distribution_GridBox(:,2) + pl.Points_GridBox(:,1))/2 ;
    else
        pl.ShiftGrid = repmat(-0.5,length(pl.GroupXAxis),1) ; 
    end
    if all(pl.ShiftGrid == 0)
        pl.ShiftGrid = pl.ShiftGrid - 0.5 ; 
    end
    [pl,inpl] = update_options(pl,inpl) ; % update
